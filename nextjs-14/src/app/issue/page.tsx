"use client";
import React, { useEffect, useState } from "react";
import IssueForm from "../components/issue/module/NewIssueForm";
import IssueList from "../components/issue/module/IssueList";
import { createEventSource } from "../components/issue/service/issue-api";
import { API } from "../components/common/enums/API";

interface Issue {
  id: number;
  title: string;
  content: string;
  law: string;
  attachment: string;
}

const App: React.FC = () => {
  const [issues, setIssues] = useState<Issue[]>([]);

  useEffect(() => {
    const eventSource = createEventSource(`${API.SERVER}`, (newIssue: Issue) => {
      setIssues((prevIssues) => [...prevIssues, newIssue]);
    });

    return () => {
      eventSource.close();
    };
  }, []);

  const handleSaveIssue = (newIssue: Issue) => {
    setIssues((prevIssues) => [...prevIssues, newIssue]);
  };

  return (
    <div className="flex justify-center h-screen w-full px-5 sm:px-0 gap-10">
      <div
        className="mt-28 w-[73vh] h-[67vh] flex rounded-[3.5vh] shadow-2xl overflow-x-auto"
        style={{ backgroundColor: "var(--form-background)" }}
      >
        <div className="w-full p-[8.5vh] justify-center items-center">
          <h1 className="text-2xl text-center font-bold">Submit an Issue</h1>
          <IssueForm onSave={handleSaveIssue} />
        </div>
      </div>
      <form className="mt-28 w-[54vh] h-[67vh]">
        <IssueList />
      </form>
    </div>
  );
};

export default App;
